class SearchController < ApplicationController
  ...
  def home
    ...
    if params[:tag]
      @solutions = Solution.tagged_with(params[:tag], on: :general_tags).includes(:user, :product)
    else
      @solutions = Solution.all.top.includes(:user, :product)
    end
    # order(cached_votes_score: :desc).order(created_at: :desc)
  end

  def recent
    @solutions = Solution.all.includes(:user, :product)
    render 'home'
  end
